/** kmsroots/examples/kms/triangle.c
 */
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
#include "kms/pixel-format.h"
#include "kms/input.h"
#include "kms/vulkan.h"
#include "kms/shader.h"

/** kmsroots/examples/meson.build
 if gpu == 'integrated'
 ...
 */
 #define VK_PHYSICAL_DEVICE_TYPE 1

#define VERTEX_SHADER_SPIRV   "~/alpine/assets/shaders/triangle-shader-vert.spv"
#define FRAGMENT_SHADER_SPIRV "~/alpine/assets/shaders/triangle-shader-frag.spv"
...
int __attribute__((visibility("default"))) backend_kms_boot(void)
{
	...
	if (create_vk_instance(&app) == -1)
		goto exit_error;
printf("create_vk_instance\n");
	if (create_kms_instance(&kms) == -1)
		goto exit_error;
printf("create_kms_instance\n");
	if (create_kms_gbm_buffers(&kms) == -1)
		goto exit_error;
printf("create_kms_gbm_buffers\n");
	if (create_kms_set_crtc(&kms) == -1)
		goto exit_error;
printf("create_kms_set_crtc\n");
	extent2D.width = kms.kmr_drm_node_display->width;
	extent2D.height = kms.kmr_drm_node_display->height;
...
printf("create_vk_framebuffers\n");
	...
	if (create_kms_atomic_request_instance(&passData, &cbuf, &fbid, &running) == -1)
		goto exit_error;
    printf("create_kms_atomic_request_instance\n");
#ifdef INCLUDE_LIBSEAT
...
printf("%s : kmr_drm_node_display_create \n",__FUNCTION__);
...
create_vk_device (struct app_vk *app, struct app_kms *kms)
{
	...
	/**
	 /backend.c:620:38: error: ¡®VK_PHYSICAL_DEVICE_TYPE¡¯ undeclared (first use in this function); did you mean ¡®VK_PHYSICAL_DEVICE_TYPE_CPU¡¯?
  620 |         phdevCreateInfo.deviceType = VK_PHYSICAL_DEVICE_TYPE;
	 
	phdevCreateInfo.deviceType = VK_PHYSICAL_DEVICE_TYPE_GPU;
	*/
