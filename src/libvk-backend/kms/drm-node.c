...
setup_atomic_modeset (int kmsfd)
{
	 ...
	 */
	/**
	 * So we should learn what are "planes",and how to use them. -- sparrow
	 */
...
printf("%s \n",__FUNCTION__);
 ...
 printf("%s : kmsNode = calloc(1, sizeof(struct kmr_drm_node)); \n",__FUNCTION__);
    if (!kmsNode) {
		kmr_utils_log(KMR_DANGER, "[x] calloc: %s", strerror(errno));
        printf("%s : kmr_utils_log(KMR_DANGER, [x] calloc:, strerror(errno)); \n",__FUNCTION__);
		return NULL;
	}
